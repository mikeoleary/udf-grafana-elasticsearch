...
LINUX_GW_IP='10.1.20.1'
EXT_VIP_IP1='10.1.10.11'
EXT_VIP_IP2='10.1.10.12'


#############
# Persist the IP address on the 2nd NIC
#############

sudo tee /etc/netplan/99-net-config.yaml << EOF
network:
  version: 2
  renderer: networkd
  ethernets:
    ens6:
     dhcp4: no
     addresses: [$LINUX_VM_IP/24]
     gateway4: $LINUX_GW_IP
EOF
...
#########################
#Install Docker and run demo app
#########################
...
# deploy demo apps
## docker run -d -p 80:80 -p 443:443 --restart unless-stopped f5devcentral/f5-demo-httpd:nginx
docker run -d -p 8081:8080 -p 8443:8443 --restart unless-stopped f5devcentral/f5-hello-world
docker run -d -p 81:8080 --restart unless-stopped nginxdemos/nginx-hello
...
###############
# Install and configure ElasticSearch, NGINX, Kibana, and Logstash
###############
...
install_kibana () {
  ...
  sudo groupadd --system elkuser
  sudo useradd -s /sbin/nologin --system -g elkuser elkuser
